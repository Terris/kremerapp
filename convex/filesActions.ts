...
export const compareImages = action({
  ...
  handler: async (ctx, args) => {
    ...
    await asyncMap(filteredImagesToCompare, async (imageSet) => {
      const image1 = allImages.page.find((image) => image._id === imageSet[0]);
      const image2 = allImages.page.find((image) => image._id === imageSet[1]);
